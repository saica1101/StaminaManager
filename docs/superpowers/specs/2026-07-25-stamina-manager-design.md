...
- NavigationViewとページ余白を除いたコンテンツ幅が720 effective px以上なら3列、
  720 effective px未満なら2列とする。通常Overviewのグリッド列数は常に2または
  3とし、実項目数や最終行の件数にかかわらず1列レイアウトへ切り替えない。
- 登録順で表示する。
...
- コンテンツ幅719/720 effective pxでの2列／3列切り替えと、最小幅でも2列を維持。
...
- 0件時に追加カードが先頭へ表示される。
- 標準幅で1行3ゲームカードを表示できる。
- 通常Overviewは最小幅でも2列を維持し、1列レイアウトへ切り替わらない。
